...
class OptionsTest < MiniTest::Spec
  ...
  describe "#call" do
    it { _(dynamic.(Object.new, 999)).must_equal({:volume =>1, :style => "Punkrock", :track => "999"}) }

    describe "static" do
      let (:static) { Declarative::Options(:volume =>1, :style => "Punkrock") }

      it { _(static.(nil)).must_equal({:volume =>1, :style => "Punkrock"}) }
    end
  end
  ...
  describe "Options() with value options" do
    ...
    it "accepts :instance_exec" do
      ...
      _(options.(context)).must_equal({:volume=>1, :style=>"Rocksteady"})
    end

    it "doesn't set :instance_exec per default" do
      style = "Metal"
      options = Declarative::Options( { volume: 1, style: lambda { |*| style } } )
      _(options.(context)).must_equal({:volume=>1, :style=>"Metal"})
    end
  end
end
